...
public class DonaTicketReqFilesDAO {
	SqlSessionFactory sessionf = SqlMapConfig.getSqlMapInterface();
	SqlSession sql_session;
	
	public DonaTicketReqFilesDAO() {
		sql_session = sessionf.openSession(true);
	}
	//게시글에 있는 첨부파일 모두 삭제
		public void delete(int req_num) {
			sql_session.delete("ReqFiles.delete", req_num);
		}
		
		//특정 게시글의 첨부파일 목록
		public List<DonaTicketFilesVO> getFiles(int req_num) {
			return sql_session.selectList("ReqFiles.getFiles", req_num);
		}
		
		public void insertFile(MultipartRequest multi, int req_num) {
			...
			while(files.hasMoreElements()) {
				...
				vo.setDona_num(req_num);
				vo.setFile_name_original(original);
				
				sql_session.insert("ReqFiles.insertFile", vo);
			}
		}
